...
שלבים קיימים (זרימה מהירה בת 2 שלבים):

1. פרטים בסיסיים: שם, שפה, שעת קימה, מצב משפחתי אופציונלי וגיל אופציונלי.
2. בחירת צעד יומי ראשון מתוך רשימה מובנית (curated daily task picker) שאפשר לבצע תוך פחות מ-5 דקות.
...
תחום הבריאות מטופל כתחום חיים סטנדרטי, באותו מבנה של מטרה → milestones → צעדים יומיים כמו כל תחום אחר. אשף הבריאות הייעודי, ה-execution plan ל-90 יום וה-health phases הוסרו (migrations v5–v7), כדי לשמור על מודל מוצר אחיד ופשוט יותר.
...
3. בוחר צעד יומי ראשון של עד 5 דקות מתוך רשימה מובנית.
4. נכנס לדשבורד.
