...
export async function POST(request: Request) {
  ...
  try {
    ...
    const plan = await openaiLifeCoachService.structureGoal({
      ...
      motivation: parsed.data.motivation,
      constraints: parsed.data.constraints,
      health_wizard_context: parsed.data.health_wizard_context,
      life_context_statuses: profile.life_context_statuses,
    ...
    return jsonOk({
      ...
      execution_plan: plan.execution_plan ?? null,
      plan_source: plan.plan_source ?? (parsed.data.health_wizard_context ? 'fallback' : undefined),
