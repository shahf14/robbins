...
export function buildGenerateExplorationQuestionsSystemPrompt(locale: AppLocale) {
  const likertRules =
    locale === 'he'
      ? [
          ...
          'דוגמה טובה: "הלחץ הכלכלי בזמן המעבר מרגיש כבד עליי לאחרונה"',
      ...
      : [
          ...
          'Good example: "Financial pressure during this transition feels heavy on me lately"',
  ...
  return [
    ...
    locale === 'he'
      ? 'משפטים מתארים תקופה לאחרונה — לא "השבוע" (טופס חד-פעמי).'
      : 'Statements describe recent times — not "this week" (one-time clarification).',
    'Input includes passive_ratings (step 3), chip_follow_ups (step 4 accuracy check), synthesis.burning_now_themes, synthesis.suppressed_by_chips.',
    ...
    'Synthesize cross-themes from the currently significant themes + exploration gaps (guilt, comparison, avoidance, hope).',
...
const FORMULATION_RULES: Record<AppLocale, string[]> = {
  he: [
    '[כללי סינתזה]',
    '1. בדיקת דיוק: נושא נכנס ל-suppressed_by_chips רק אם נשאל בשלב 4 והמשתמש בחר "בכלל לא". נושא גבוה בשלב 3 שלא נשאל בשלב 4 — נשאר תקף.',
    ...
    '   stressors = מה מגביר את הקושי כרגע (מ-exploration_ratings גבוהים). לא לחזור על נושאי הליבה.',
    ...
    '5. שיקוף חייב לשקף רק את הנושאים שנשארו משמעותיים אחרי בדיקת הדיוק — לא suppressed.',
  ],
  en: [
    '[Synthesis rules]',
    '1. Accuracy check: a theme is suppressed ONLY if asked in Phase 4 and user chose "not at all". Phase 3 themes not asked in Phase 4 remain valid.',
    ...
    '   stressors = what amplifies the difficulty right now (from high exploration_ratings). Do not repeat core topics.',
    ...
    '5. Reflection must reflect only themes that remain significant after the accuracy check — not suppressed.',
  ],
};
...
export function buildMicroGoalSystemPrompt(locale: AppLocale) {
  const analysis =
    locale === 'he'
      ? [
          ...
          'שלב 1: קבע burning_focus — משפט אחד: מה הכי חשוב להקל עליו השבוע.',
          'שלב 2: צור בדיוק 5 goal_options:',
          '  (1) goal_type=practical — פעולה פרקטית שמפחיתה את המוקד המרכזי (ראה goal_alignment.practical_must_reduce).',
          '  (2) goal_type=mindset — עובד על maintaining_factors (goal_alignment.mindset_must_address), לא על המוקד המרכזי ישירות.',
          ...
          'אסור יעדי wellness גנריים (מדיטציה, שינה, נשימות) אלא אם שינה היא נושא משמעותי כרגע ולא סומנה כפחות רלוונטית.',
      ...
      : [
          ...
          'Step 1: Set burning_focus — one sentence for what matters most to ease this week.',
          'Step 2: Create exactly 5 goal_options:',
          '  (1) goal_type=practical — concrete action that reduces the central focus (see goal_alignment.practical_must_reduce).',
          '  (2) goal_type=mindset — addresses maintaining_factors (goal_alignment.mindset_must_address), not the central focus directly.',
          ...
          'No generic wellness goals unless sleep is a significant current theme and not suppressed.',
  ...
  return [
    ...
    'Small and kind — not personality change. Every goal must clearly ease the clarified central focus.',
    ...analysis,
  ].join('\n');
}
