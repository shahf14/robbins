...
export function ProgressBar({current, total}: {current: number; total: number}) {
  ...
  return (
    <div
      className="mb-6 h-1 w-full rounded-full bg-white/10"
      role="progressbar"
      aria-valuenow={pct}
      aria-valuemin={0}
      aria-valuemax={100}
  ...
  return (
    <div className="mx-auto max-w-xl space-y-8 py-12 text-center">
      <div className="space-y-3">
        ...
        <p className="text-white/60">{t('start.subtitle')}</p>
        <div className="mx-auto mt-4 max-w-md space-y-2 text-start">
          ...
          <p className="px-1 text-xs leading-5 text-white/45">{t('start.notJournal')}</p>
        </div>
        ...
        <div className="mx-auto mt-5 grid max-w-md gap-3 sm:grid-cols-3">
          {(['quick', 'standard', 'deep'] as EveningMode[]).map((key) => (
            <button
              ...
              className="focus-ring group rounded-xl border border-white/10 bg-white/4 px-3 py-4 text-start transition hover:border-violet-400/40 hover:bg-violet-500/7"
            >
              <div className="flex items-center justify-between gap-2">
                <p className="text-sm font-bold text-white">{t(`mode.${key}`)}</p>
                <span className="shrink-0 rounded-full border border-white/15 bg-black/25 px-2 py-0.5 text-[10px] font-bold text-white/85">
              ...
              <p className="mt-2 text-[11px] leading-4 text-white/55">
                <span className="font-semibold text-white/65">{t('mode.outcomeLabel')}: </span>
                {t(`mode.${key}Outcome`)}
              </p>
      ...
      {completedCount > 0 && (
        <div className="flex items-center justify-center gap-8">
          <div className="text-center">
            ...
            <p className="text-xs text-white/40">{t('start.streak')}</p>
          </div>
          <div className="text-center">
            <p className="text-2xl font-bold">{completedCount}</p>
            <p className="text-xs text-white/40">{t('start.completed')}</p>
          </div>
        </div>
      )}

      {completedCount === 0 && (
        <div className="rounded-xl border border-white/10 p-5 text-left space-y-2">
          <p className="text-xs font-semibold uppercase tracking-widest text-white/40">
  ...
  return (
    <div className="mx-auto max-w-xl space-y-6 py-8">
      <div className="text-center space-y-2">
        ...
        {suggestKey ? (
          <p className="text-sm leading-7 text-white/55">{tRoot(suggestKey)}</p>
        ) : null}
      </div>
      <div className="space-y-3">
        {modes.map((m) => (
          <button
            ...
            type="button"
            onClick={() => onSelect(m)}
            className={`focus-ring w-full rounded-xl border p-4 text-left transition hover:border-white/20 hover:bg-white/10 ${
              m === suggestedMode
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                ...
                <p className="mt-1 text-sm text-white/50">{t(`mode.${m}Desc`)}</p>
