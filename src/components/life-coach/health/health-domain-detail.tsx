...
function HealthDetailContent() {
  ...
  const {activeTab, setActiveTab} = useDomainDetailUrlState({
    fallbackTab: loading ? 'today' : hasHealthGoals ? 'today' : 'goal',
    ready: !loading,
    includeStepFilter: false,
  });
  ...
  async function handleGenerateSteps() {
    ...
    try {
      const steps = await generateDomainDailySteps(locale, 'health', healthSteps.length > 0);
      await refresh();
      if (steps.length === 0) {
  ...
  const domainLabel = t('lifeCoach.domains.health.short');
  const generateLabel =
